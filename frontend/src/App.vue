    ...
    <section v-if="activeView === 'chat'" class="page page-chat">
      <div class="page-heading">
        <div class="heading-copy">
          ...
          <h1>企业运营协同工作台</h1>
          <p>输入运营请求，查看意图识别、知识检索、Agent 路由与执行轨迹。</p>
        </div>
        ...
        <section class="chat-stage">
          ...
          <div class="messages" ref="messageList">
            ...
            <div v-if="messages.length === 0" class="empty-state">
              ...
              <h2>从一条业务请求开始</h2>
              <p>快捷问题覆盖单 Agent、知识增强和跨领域协同三类演示。</p>
              <div class="starter-prompts">
                <button @click="usePrompt('我想申请退款，订单号是 #12345')">退款申请</button>
                <button @click="usePrompt('登录一直报 401，应该怎么排查？')">技术排查</button>
                <button @click="usePrompt('登录一直报 401，而且刚才还被重复扣款了')">跨域协同</button>
              </div>
            </div>
          </div>
          ...
          <div class="chat-sidebar-scroll">
            ...
            <section class="side-card connection-card">
              ...
              <div class="backend-tabs">
                <button class="active" type="button">Coordexa Python API</button>
              </div>
            ...
            <section class="side-card trace-card">
              ...
              <div v-if="lastResponse" class="trace-body">
                ...
                <dl class="detail-list">
                  <div><dt>主 Agent</dt><dd>{{ formatAgent(lastResponse.primaryAgent || lastResponse.agentType) }}</dd></div>
            ...
            <section class="side-card monitor-card">
              ...
              <div v-if="activeAlerts.length" class="alert-list">
                <div v-for="alert in activeAlerts.slice(0, 3)" :key="`${alert.metric}-${alert.ts}`" class="alert-note">
                  {{ alert.message || alert.detail || alert.title || alert.metric }}
                </div>
              </div>
    ...
    <section v-else-if="activeView === 'knowledge'" class="page page-knowledge">
      <div class="page-heading">
        <div class="heading-copy">
          ...
          <h1>知识库</h1>
          <p>搜索、补充和维护运营 Agent 使用的企业知识片段。</p>
        </div>
