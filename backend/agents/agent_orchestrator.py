...
class Request:
    ...
    knowledge_prefetched: bool = False
...
class BaseAgent:
    ...
    async def _call_llm(self, req: Request) -> str:
        ...
        # 已在 API 层预取知识时避免重复检索；非业务意图不暴露 RAG 工具。
        if req.knowledge_prefetched or req.intent in {
    ...
    def _needs_escalation(self, content: str) -> bool:
        """只把明确的升级行动建议视为升级，避免能力介绍中的关键词误报。"""
        patterns = (
            r"(?:需要|需|建议|请|应当|必须).{0,12}(?:转人工|人工客服|转接.{0,4}客服|升级处理)",
            r"(?:无法|不能).{0,8}(?:继续)?处理",
            r"\b(?:please|should|must|need to)\s+(?:escalate|contact\s+(?:a\s+)?(?:human|specialist))\b",
        )
...
class GeneralAgent(BaseAgent):
    ...
    system_prompt = (
        "你是 Coordexa 运营协调 Agent。友好、简洁地回答企业运营相关问题。"
...
class TechnicalAgent(BaseAgent):
    ...
    system_prompt = (
        "你是 Coordexa 技术可靠性 Agent。专注于：故障排查、错误诊断、系统配置。"
...
class BillingAgent(BaseAgent):
    ...
    system_prompt = (
        "你是 Coordexa 收入与合规 Agent。专注于：账单查询、退款申请、发票问题、订阅管理。"
...
class EscalationAgent(BaseAgent):
    ...
    system_prompt = "你负责 Coordexa 运营升级交接，不要继续模拟已完成的后台操作。"
...
class AgentOrchestrator:
    ...
    def __init__(
        ...
        self._shared_tools: Dict[str, AgentToolSpec] = {}
        self._recent_tool_traces = deque(maxlen=_env_int("COORDEXA_TOOL_TRACE_MAX", 200))
    ...
    def _make_agent(
        ...
        env_name = f"COORDEXA_{agent_cls.agent_type.value.upper()}_MODEL"
    ...
    async def run(self, req: Request) -> OrchestratorResult:
        ...
        # 4. 升级检查
        escalated = decision.primary_agent is AgentType.ESCALATION
        if escalated:
