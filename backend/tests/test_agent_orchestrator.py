...
def test_escalation_detection_requires_an_explicit_action_statement():
    agent = GeneralAgent(FakeClient(), "test-model")

    assert agent._needs_escalation("投诉与升级问题可以帮你梳理，必要时转人工处理。") is False
    assert agent._needs_escalation("该问题需要转人工客服进一步处理。") is True
    assert agent._needs_escalation("抱歉，我无法处理该请求。") is True
