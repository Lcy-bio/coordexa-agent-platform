...
返回格式: {{"preferences": ["..."], "entities": {{"产品": [], "问题类型": []}}}}"""
    ...
    async def _compress(self, user_id: str, conv_id: str) -> None:
        ...
        try:
            ...
            logger.warning(f"生成摘要失败，使用确定性回退: {ex}")
    ...
    def _parse_profile_payload(raw: str) -> Dict[str, Any]:
        """兼容代码块/说明文字，并把画像限制为可持久化的最小结构。"""
        payload = extract_json_value(raw, dict)

        preferences = payload.get("preferences", [])
        if not isinstance(preferences, list):
            preferences = [preferences]
        preferences = [str(item).strip() for item in preferences if str(item).strip()][:20]
        ...
        if isinstance(raw_entities, dict):
            for key, value in raw_entities.items():
                ...
                cleaned = [str(item).strip() for item in values if str(item).strip()][:20]
                if cleaned:
                    entities[str(key).strip()] = cleaned
