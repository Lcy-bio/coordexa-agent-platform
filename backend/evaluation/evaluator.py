    ...
    async def judge(
        ...
        for attempt in range(2):
            try:
                ...
                if not raw.strip():
                    logger.warning(
                        "LLM Judge 返回空文本: attempt=%s max_tokens=%s stop_reason=%s usage=%s",
                        attempt + 1,
                        max_tokens,
                ...
                last_error = ex
                if attempt == 0:
                    logger.warning(f"LLM Judge 首次失败，重试一次: {ex}")

        logger.warning(f"LLM Judge 失败: {last_error}")
        return QualityScores(
            0.5, 0.5, 0.5, 0.5,
            judge_failed=True,
...
class EndToEndEvaluator:
    ...
    async def run(
            ...
            for i, case in enumerate(dialog_cases):
                ...
                for r in case_results:
                    ...
                    if r.metadata.get("judge_failed"):
                        # Judge 解析失败属于评测基础设施故障，不能当作有效的 0.5 分。
                        continue
                    judge_successful += 1
                    for k in all_scores:
                        if k in r.scores:
        ...
        if judge_total:
            avg_scores["judge_success_rate"] = round(judge_successful / judge_total, 4)
    ...
    async def _evaluate_dialog_case(self, case: Dict[str, Any], case_idx: int) -> List[EvalResult]:
        ...
        for turn_idx, question in enumerate(questions):
            ...
            intent_result = await self._recognizer.recognize(
                question,
    ...
    def _recommendations(
        ...
        if scores.get("judge_success_rate", 1.0) < 0.95:
            recs.append("Judge 成功率 < 95%：检查模型返回格式、JSON 解析和兼容接口稳定性")
        if not recs:
