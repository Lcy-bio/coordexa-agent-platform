...
只返回 JSON 数组，不要其他文字。"""
        ...
        try:
            ...
            order: List[int] = []
            for value in parsed_order:
                if isinstance(value, int) and not isinstance(value, bool) and 0 <= value < len(items):
                    if value not in order:
                        order.append(value)
            order.extend(index for index in range(len(items)) if index not in order)
    ...
    def _validate_params(self, tool: Tool, params: Dict[str, Any]) -> None:
        ...
        if schema.get("additionalProperties") is False:
            ...
            if unknown:
                raise ValueError(f"工具 {tool.name} 不允许参数: {', '.join(unknown)}")
            ...
            if key in properties:
                ...
                if isinstance(value, (int, float)) and not isinstance(value, bool):
                    ...
                    if "maximum" in properties[key] and value > properties[key]["maximum"]:
                        raise ValueError(f"工具 {tool.name} 参数 {key} 超过最大值")
