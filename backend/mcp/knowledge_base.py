...
class KnowledgeBase:
    ...
    def __init__(
        ...
        self._collection = self._client.get_or_create_collection(
            ...
            metadata={"description": "Coordexa 企业运营知识库"},
        )
    ...
    def add_documents(self, documents: List[Dict[str, str]]) -> int:
        ...
        if ids:
            ...
            logger.info(f"知识库写入 {len(ids)} 个文档片段")
    ...
    def search(self, query: str, top_k: int = 5) -> List[Dict[str, Any]]:
        ...
        if total <= 0:
            return []
        candidate_k = min(total, max(top_k * 3, top_k))
        ...
        vector_values = [item["vector_score"] for item in items]
        low, high = min(vector_values), max(vector_values)
        for item in items:
            vector_normalized = (
                (item["vector_score"] - low) / (high - low)
    ...
    def _lexical_score(cls, query: str, title: str, content: str) -> float:
        """基于字符二/三元组计算中文友好的词面覆盖分。"""
        query_tokens = cls._char_ngrams(query)
        if not query_tokens:
            return 0.0
        title_tokens = cls._char_ngrams(title)
        document_tokens = cls._char_ngrams(f"{title} {content}")
        ...
        return min(1.0, 0.65 * query_coverage + 0.35 * title_coverage)
