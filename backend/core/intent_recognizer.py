...
可选意图: {", ".join(c.value for c in IntentCategory)}"""
    ...
    def _vote(self, llm: Dict, emb: Dict, pat: Dict) -> tuple[IntentCategory, float, Dict[str, float]]:
        ...
        # “转人工/人工客服”是明确的执行边界，细粒度意图应优先于模型给出的
        # escalation 宽泛分类，避免继续进入普通问答链路。
        if (
            pat.get("intent") == IntentCategory.HUMAN_HANDOFF
            and source_scores["pattern"] >= 0.5
        ):
            source_scores["explicit_handoff"] = source_scores["pattern"]
            return IntentCategory.HUMAN_HANDOFF, source_scores["pattern"], source_scores
        if llm.get("failed"):
    ...
    def _extract_entities(self, message: str) -> Dict[str, List[str]]:
        ...
        message = self._clean_text(message)
        return {
            "order_id": self._unique(re.findall(r"(?:订单号?|order(?:_id)?|#)\s*[:：#]?\s*(?:是|为)?\s*([A-Za-z0-9_-]{4,32})", message, re.I)),
            "product": [],
    ...
    def _parse_llm_payload(raw: str) -> Dict[str, Any]:
        """Accept strict JSON and common ``intent: value`` compatibility output."""
        try:
            data = extract_json_value(raw, dict)
            return dict(data)
        except ValueError:
            choices = sorted((intent.value for intent in IntentCategory), key=len, reverse=True)
            alternation = "|".join(re.escape(choice) for choice in choices)
            intent_match = re.search(
                rf"(?i)(?:intent|意图)\s*[:=：]\s*[\"']?({alternation})[\"']?",
                raw or "",
            )
            if not intent_match:
                raise
            numbers = extract_labeled_floats(raw, ("confidence", "置信度"))
            confidence = numbers.get("confidence", numbers.get("置信度", 0.0))
            return {
