"""Coordexa 企业运营多 Agent 协同平台 — FastAPI 入口。"""
import asyncio
import logging
import os
import pathlib
...
async def lifespan(app: FastAPI):
    ...
    _skill_manager = SkillManager(
        ...
        max_prompt_chars=int(os.getenv(
            "COORDEXA_SKILLS_MAX_PROMPT_CHARS",
            "5000",
        )),
    )
    _skill_manager.load()
    ...
    logger.info("Coordexa 已就绪")
    yield

    await _monitor.stop()
    if _memory is not None:
        await _memory.close()
    logger.info("Coordexa 已关闭")


# ── FastAPI ───────────────────────────────────────────────────────────────────
app = FastAPI(
    title="Coordexa 企业运营 Agent API",
    description="面向企业运营请求的知识增强、多 Agent 协同与评测接口。",
    version="1.0.0",
    lifespan=lifespan,
    docs_url="/docs",
)
...
async def _chat_impl(req: ChatRequest, request_id: str):
    ...
    # 业务类请求显式触发知识检索；寒暄、反馈、转人工等请求跳过。
    # 这一步把“意图门控 RAG”放进真实 /chat 主链路，避免仅依赖模型自主选工具。
    knowledge_context, knowledge_used = await _build_knowledge_context(
        req.message,
        intent=intent_result.intent,
    )
    context_parts = [part for part in (mem_ctx.to_prompt_text(), knowledge_context) if part]
    full_context = "\n\n".join(context_parts)
    ...
    return ChatResponse(
        ...
        tools_used=list(dict.fromkeys(
            (["knowledge_search"] if knowledge_used else []) + result.tools_used
        )),
        routing_reason=result.routing_reason,
        routing_confidence=result.routing_confidence,
        escalated=result.escalated,
        latency_ms=round(result.latency_ms, 1),
        knowledge_used=knowledge_used or "search_knowledge_base" in result.tools_used,
        entities=intent_result.entities,
...
@app.post("/search")
async def search(query: str, top_k: int = Query(5, ge=1, le=20)):
    """
...
async def upload_knowledge(file: UploadFile = File(...)):
    ...
    if suffix == ".json":
        ...
        try:
            raw_docs = _json.loads(text)
            if not isinstance(raw_docs, list):
                raise HTTPException(400, "JSON 文件应为数组格式: [{title, content}, ...]")
            docs = [DocInput.model_validate(item).model_dump() for item in raw_docs]
        except _json.JSONDecodeError as e:
            raise HTTPException(400, f"JSON 解析失败: {e}")
        except ValidationError as e:
            raise HTTPException(400, f"JSON 文档字段校验失败: {e.errors()}")
    else:
        # txt / md：整个文件作为一篇文档
...
async def _cli():
    ...
    while True:
        ...
        print(f"\nCoordexa [{result.agent_type.value}]: {result.response}\n")
